...
![Attacker IP and user-agent](q1and2.png)
...
> **Answer (submitted & accepted):** `198.23.200.101`
>
> ⚠️ **Map this to the exact wording on screen.** Both IPs matter and the two IP questions (Q1 and Q6) use them. In this writeup: `198.23.200.101` = the IP that logged in with valid creds; `198.51.100.100` = the brute-force source / "other" IP that also produced a webadmin success. If one is rejected, swap them.
...
![Find All on "not found" — 9 matches](q3.png)
...
![ftp — incorrect password](q4p1.png)
![webadmin — success](q4p2.png)
![websitemanager — incorrect password](q4p3.png)
...
![Successful webadmin login](q5and7.png)

> **Answer:** `webadmin`

---

### Q6 — The other IP address that logged into this account *(IP question)* — Format: `X.X.X.X`
...
![Successful login from 198.23.200.101](q6.png)

Since Q1's accepted answer was `198.23.200.101`, the "other" IP that also logged into the account is the brute-force source.

> **Answer:** `198.51.100.100`
>
> ⚠️ Same caveat as Q1 — confirm the wording. The two IPs fill the two IP questions; if one is rejected, swap.
...
![Endpoint /api/login visible on the success event](q5and7.png)

> **Answer:** `/api/login`

---

### Q8 — Timestamp of the earliest authentication attempt *(time question)*
...
![Entry showing the 10:00:10 timestamp](q8.png)

> **Answer (verify exact wording/format):** `2023-06-29T10:00:10`
>
> ⚠️ This is the question I'm least certain about — confirm whether it wants the *first attempt*, the *successful login time*, or a count, and match the expected format on screen.
...
![CyberChef From Base64 → webadmin1234](q9.png)
...
| 8 | Earliest authentication attempt | `2023-06-29T10:00:10` *(verify wording/format)* |
