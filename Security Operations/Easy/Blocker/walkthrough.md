...
**Method:** In the General tab of the 3:55:16 PM event, read the `Image` field — this is the process that attempted to write the blocked file.
...
```
This is the 4:04:34 PM event. Read its `TargetFilename`:
```
C:\Users\BTLOTest\Downloads\freebl3.dll
```
Note the initiating Image here is `powershell.exe`, and the blocked file is a **.dll** — confirming that FileBlockExecutable catches more than just `.exe`.
